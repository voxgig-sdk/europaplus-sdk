...
[MCP](https://modelcontextprotocol.io) server exposing the Europaplus SDK as
two agent tools — `europaplus_list` and `europaplus_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/europaplus-mcp)
make build

# 2. Provide credentials via the environment
export EUROPAPLUS_APIKEY=sk_live_xxx

# 3a. Install into Claude Code over stdio (most common)
claude mcp add --scope user europaplus \
  -- /absolute/path/to/europaplus-mcp -transport stdio

# 3b. …or run a shared HTTP server instead
./europaplus-mcp -transport http -addr :8080
```

Tool-call arguments (what an agent sends):

```jsonc
// europaplus_list: first page of records
{ "entity": "schedule" }
{ "entity": "schedule", "query": { } }

// europaplus_load: one record by id
{ "entity": "schedule", "query": { "id": 1 } }
```
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/europaplus-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user europaplus \
     -- "$PWD"/dist/*/europaplus-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `europaplus_list` and `europaplus_load` tools now appear
   in new sessions. Ask the agent to *"list schedule using europaplus"*
   and it calls `europaplus_list` with `{"entity":"schedule"}`.
...
```sh
export EUROPAPLUS_APIKEY=sk_live_xxx            # API key
export EUROPAPLUS_BASE=https://api.example.com  # optional: override the API base URL
```
...
| Tool | Args | Returns |
|------|------|---------|
| `europaplus_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `europaplus_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `EUROPAPLUS_APIKEY` | API key sent with every request. |
| `EUROPAPLUS_BASE` | Optional override of the API base URL. |
...
```sh
./europaplus-mcp -transport http -addr :18080 &
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"europaplus_load","arguments":{"entity":"schedule","query":{"id":1}}}}'
```
