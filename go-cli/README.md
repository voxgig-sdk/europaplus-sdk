...
boru-driven command-line client **and** interactive REPL for the Europaplus
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/europaplus-cli)
make build
...
./europaplus-cli list schedule

# 5. Override the API base URL for a single call
EUROPAPLUS_BASE=https://api.example.com ./europaplus-cli list schedule

# 6. No arguments -> interactive REPL
./europaplus-cli
europaplus> list schedule
europaplus> /quit
```
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/europaplus-cli
   ```
...
3. **Run a query.** Evaluate an boru expression against the API (or run with no
   ...
   ```sh
   ./dist/*/europaplus-cli list schedule
   ```
...
Run with no arguments to open a REPL (prompt `europaplus>`). Each line is
...
- `<entity>` is a bareword, auto-quoted as an boru atom (e.g. `schedule`).
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/europaplus-cli`. |
...
(`./europaplus-cli <expr>`) and interactively (the REPL), and expressions compose the
