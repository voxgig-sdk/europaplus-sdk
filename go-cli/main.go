// Command Europaplus-cli is a CLI + REPL for the Europaplus SDK,
...
const prompt = "europaplus"

// entitiesHelp is the space-separated entity list shown by /help.
const entitiesHelp = "schedule"
...
func usage(out io.Writer) {
	...
	fmt.Fprintln(out, "Env:      EUROPAPLUS_APIKEY (api key), EUROPAPLUS_BASE (base url override)")
...
func run(args []string, in io.Reader, out, errOut io.Writer) int {
	...
	// Configure from the environment: EUROPAPLUS_APIKEY carries the API key and
	// EUROPAPLUS_BASE optionally overrides the API base URL (e.g. production).
	// Both injectable by a secrets vault. Unset -> nil config defaults.
	var opts map[string]any
	if apikey := os.Getenv("EUROPAPLUS_APIKEY"); apikey != "" {
		opts = map[string]any{"apikey": apikey}
	}
	if base := os.Getenv("EUROPAPLUS_BASE"); base != "" {
		if opts == nil {
			opts = map[string]any{}
		}
		opts["base"] = base
	}
	client := sdk.NewEuropaplusSDK(opts)
