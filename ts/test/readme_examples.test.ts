...
//     (\`new EuropaplusSDK(...)\` / \`EuropaplusSDK.test(...)\` become
//     \`EuropaplusSDK.test({ entity: ... })\`), transpiled to JS, and run
...
const TEST_SEED = {"entity":{"schedule":{"test01":{"id":"test01"}}}}
...
// rewrite real client construction (\`new EuropaplusSDK(...)\` and any
// \`EuropaplusSDK.test(...)\`) to a seeded test-mode client, and give
...
function rewriteForRun(code: string): string {
  ...
  out = out.replace(/new\s+EuropaplusSDK\s*\([^)]*\)/g, () => SEEDED_CTOR)
  out = out.replace(/EuropaplusSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR)
