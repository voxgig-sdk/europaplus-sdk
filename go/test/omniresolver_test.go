...
import (
	...
	omni "github.com/voxgig-sdk/europaplus-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.EuropaplusError); is {
				out := map[string]any{
					"name":    "EuropaplusError",
...
func omniCtx(arg any, client *sdk.EuropaplusSDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.EuropaplusSDK); is {
			client = live
