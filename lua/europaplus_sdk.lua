...
local EuropaplusSDK = {}
EuropaplusSDK.__index = EuropaplusSDK
...
EuropaplusSDK._make_feature = _make_feature


function EuropaplusSDK.new(options)
  local self = setmetatable({}, EuropaplusSDK)
  self.mode = "live"
  self.features = {}
  self.options = nil
...
function EuropaplusSDK:options_map()
...
function EuropaplusSDK:get_utility()
  return Utility.copy(self._utility)
end


function EuropaplusSDK:get_root_ctx()
  return self._rootctx
end


function EuropaplusSDK:prepare(fetchargs)
...
function EuropaplusSDK:direct(fetchargs)
...
-- by passing it.
function EuropaplusSDK:_raw_request(fetchargs)
  local utility = self._utility
...
function EuropaplusSDK:graphql(query, variables, ctrl)
  ...
  if type(errors) == "table" and 0 < #errors then
    ...
    res.ok = false
    res.err = "EuropaplusSDK: graphql: " .. msg
    res.graphql = errors
  end
...
function EuropaplusSDK.test(testopts, sdkopts)
  sdkopts = sdkopts or {}
  ...
  local sdk = EuropaplusSDK.new(sdkopts)
  sdk.mode = "test"
