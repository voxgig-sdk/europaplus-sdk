...
class EuropaplusTestRunner:
    ...
    def load_env_local():
        ...
        for line in content.splitlines():
            ...
            EuropaplusTestRunner._env[key] = val

    @staticmethod
    def getenv(key):
        val = EuropaplusTestRunner._env.get(key)
        if val is not None:
            return val
    ...
    @staticmethod
    def env_override(m):
        live = EuropaplusTestRunner.getenv("EUROPAPLUS_TEST_LIVE")
        override = EuropaplusTestRunner.getenv("EUROPAPLUS_TEST_OVERRIDE")

        if live == "TRUE" or override == "TRUE":
            for key in list(m.keys()):
                envval = EuropaplusTestRunner.getenv(key)
        ...
        explain = EuropaplusTestRunner.getenv("EUROPAPLUS_TEST_EXPLAIN")
        if explain is not None and explain != "":
            m["EUROPAPLUS_TEST_EXPLAIN"] = explain
    ...
    def load_test_control():
        ...
        """
        if EuropaplusTestRunner._test_control is not None:
            return EuropaplusTestRunner._test_control
        ctrl_path = os.path.join(os.path.dirname(__file__), "sdk-test-control.json")
        try:
            with open(ctrl_path, "r") as f:
                EuropaplusTestRunner._test_control = json.load(f)
        except (FileNotFoundError, IOError, ValueError):
            EuropaplusTestRunner._test_control = {
                "version": 1,
                "test": {"skip": {
    ...
    def is_control_skipped(kind, name, mode):
        ...
        ctrl = EuropaplusTestRunner.load_test_control()
...
def load_env_local():
    EuropaplusTestRunner.load_env_local()


def env_override(m):
    return EuropaplusTestRunner.env_override(m)


def entity_data(v):
    return EuropaplusTestRunner.entity_data(v)


def entity_list_to_data(lst):
    return EuropaplusTestRunner.entity_list_to_data(lst)


def is_control_skipped(kind, name, mode):
    return EuropaplusTestRunner.is_control_skipped(kind, name, mode)


def load_test_control():
    return EuropaplusTestRunner.load_test_control()


def live_client_options():
    return EuropaplusTestRunner.live_client_options()


def live_delay_ms():
    return EuropaplusTestRunner.live_delay_ms()
