...
require_relative 'Europaplus_types'
...
class EuropaplusSDK
  ...
  def initialize(options = {})
    ...
    feature_opts = EuropaplusHelpers.to_map(VoxgigStruct.getprop(@options, "feature"))
    if feature_opts
      featureorder = VoxgigStruct.getpath(@options, "__derived__.featureorder")
      if featureorder.is_a?(Array)
        featureorder.each do |fname|
          fopts = EuropaplusHelpers.to_map(feature_opts[fname])
          if fopts && fopts["active"] == true
            utility.feature_add.call(@_rootctx, EuropaplusFeatures.make_feature(fname))
          end
        end
      end
    end
  ...
  def get_utility
    EuropaplusUtility.copy(@_utility)
  end
  ...
  def prepare(fetchargs = {})
    ...
    ctrl = EuropaplusHelpers.to_map(VoxgigStruct.getprop(fetchargs, "ctrl")) || {}
    ...
    params = EuropaplusHelpers.to_map(VoxgigStruct.getprop(fetchargs, "params")) || {}
    query = EuropaplusHelpers.to_map(VoxgigStruct.getprop(fetchargs, "query")) || {}
  ...
  def op_denied(op)
    ...
    {
      "ok" => false,
      "err" => EuropaplusError.new(
        "#{op}_allow",
        "EuropaplusSDK: #{op}: operation not allowed by" \
  ...
  def raw_request(fetchargs = {})
    ...
    if fetched.is_a?(Hash)
      status = EuropaplusHelpers.to_int(VoxgigStruct.getprop(fetched, "status"))
  ...
  def graphql(query, variables = nil, ctrl = nil)
    ...
    if errors.is_a?(Array) && !errors.empty?
      ...
      res["ok"] = false
      res["err"] = EuropaplusError.new(
        "graphql_error", "EuropaplusSDK: graphql: #{msg}")
      res["graphql"] = errors
    end
