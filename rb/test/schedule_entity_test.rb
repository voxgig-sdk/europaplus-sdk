...
class ScheduleEntityTest < Minitest::Test
  ...
  def test_stream
    ...
    base = EuropaplusSDK.test(seed, nil)
  ...
  def test_basic_flow
    ...
    if setup[:synthetic_only]
      skip "live entity test uses synthetic IDs from fixture — set EUROPAPLUS_TEST_SCHEDULE_ENTID JSON to run live"
      return
    end
    client = setup[:client]
...
def schedule_basic_setup(extra)
  ...
  entid_env_raw = ENV["EUROPAPLUS_TEST_SCHEDULE_ENTID"]
  ...
  if env["EUROPAPLUS_TEST_LIVE"] == "TRUE"
    ...
    client = EuropaplusSDK.new(Helpers.to_map(merged_opts))
  end
