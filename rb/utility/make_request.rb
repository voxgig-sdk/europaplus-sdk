...
module EuropaplusUtilities
  MakeRequest = ->(ctx) {
    ...
    spec = ctx.spec
    utility = ctx.utility
    response = EuropaplusResponse.new({})
    ...
    if fetch_err
      ...
      response = EuropaplusResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = EuropaplusResponse.new(fetched)
    else
