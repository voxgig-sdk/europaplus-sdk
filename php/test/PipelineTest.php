...
require_once __DIR__ . '/../europaplus_sdk.php';
...
{
    private static function utility(): EuropaplusUtility
    {
        return new EuropaplusUtility();
    }
    ...
    private static function ctx(array $over = []): EuropaplusContext
    {
        $utility = $over['utility'] ?? self::utility();
        $client = $over['client'] ?? new PlClient(['base' => 'http://h']);
        $ctx = new EuropaplusContext([
            'client' => $client,
            'utility' => $utility,
        ], null);
        $ctx->op = new EuropaplusOperation(['name' => 'load', 'entity' => 'x']);
        foreach ($over as $k => $v) {
    ...
    {
        return ($err instanceof EuropaplusError) ? $err->sdk_code : '';
    }
    ...
    public function test_make_point_rejects_a_disallowed_operation(): void
    {
        ...
        $ctx->op = new EuropaplusOperation(['name' => 'nope', 'entity' => 'x']);
        [$point, $err] = EuropaplusMakePoint::call($ctx);
    ...
    public function test_make_point_rejects_an_operation_with_no_endpoints(): void
    {
        ...
        $ctx->op = new EuropaplusOperation(['name' => 'load', 'entity' => 'x', 'points' => []]);
        [$point, $err] = EuropaplusMakePoint::call($ctx);
        $this->assertNull($point);
    ...
    public function test_make_point_returns_the_single_point(): void
    {
        ...
        $ctx = self::ctx(['options' => ['allow' => ['op' => 'load,list,create,update,remove']]]);
        $ctx->op = new EuropaplusOperation(['name' => 'load', 'entity' => 'x', 'points' => [$point]]);
        [$got, $err] = EuropaplusMakePoint::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new EuropaplusSpec(['method' => 'GET']);
        $ctx = self::ctx();
        $ctx->out['spec'] = $preset;
        [$got, $err] = EuropaplusMakeSpec::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_response_guards_missing_spec_response_result(): void
    {
        $ctx = self::ctx([
            'spec' => null,
            'response' => new EuropaplusResponse([]),
            'result' => new EuropaplusResult([]),
        ]);
        [, $err] = EuropaplusMakeResponse::call($ctx);
        $this->assertSame('response_no_spec', self::code($err));

        $ctx = self::ctx([
            'spec' => new EuropaplusSpec([]),
            'response' => null,
            'result' => new EuropaplusResult([]),
        ]);
        [, $err] = EuropaplusMakeResponse::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'spec' => new EuropaplusSpec(['step' => 's']),
            'response' => new EuropaplusResponse(self::resp(404, null, ['x-a' => '1'])),
            'result' => new EuropaplusResult([]),
        ]);
        [, $err] = EuropaplusMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_2xx_parses_the_body_and_marks_ok(): void
    {
        $ctx = self::ctx([
            'spec' => new EuropaplusSpec(['step' => 's']),
            'response' => new EuropaplusResponse(self::resp(200, ['v' => 1])),
            'result' => new EuropaplusResult([]),
        ]);
        [, $err] = EuropaplusMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_records_to_ctrl_explain_when_explain_is_on(): void
    {
        $ctx = self::ctx([
            'spec' => new EuropaplusSpec(['step' => 's']),
            'response' => new EuropaplusResponse(self::resp(200, ['v' => 2])),
            'result' => new EuropaplusResult([]),
        ]);
        $ctx->ctrl->explain = ['on' => true];
        EuropaplusMakeResponse::call($ctx);
    ...
    {
        $preset = new EuropaplusResponse(self::resp(299));
        $ctx = self::ctx([
            'spec' => new EuropaplusSpec([]),
            'response' => new EuropaplusResponse([]),
            'result' => new EuropaplusResult([]),
        ...
        [$got, $err] = EuropaplusMakeResponse::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null, 'result' => new EuropaplusResult([])]);
        [, $err] = EuropaplusMakeResult::call($ctx);
        $this->assertSame('result_no_spec', self::code($err));

        $ctx = self::ctx(['spec' => new EuropaplusSpec([]), 'result' => null]);
        [, $err] = EuropaplusMakeResult::call($ctx);
    ...
    public function test_make_result_list_op_wraps_resdata_into_entity_instances(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new EuropaplusSpec(['step' => 's']),
            'result' => new EuropaplusResult(['ok' => true, 'resdata' => [['a' => 1], ['a' => 2]]]),
        ]);
        $ctx->op = new EuropaplusOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = EuropaplusMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_result_empty_list_yields_empty_resdata(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new EuropaplusSpec(['step' => 's']),
            'result' => new EuropaplusResult(['ok' => true, 'resdata' => []]),
        ]);
        $ctx->op = new EuropaplusOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = EuropaplusMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new EuropaplusResult(['ok' => true]);
        $ctx = self::ctx([
            'spec' => new EuropaplusSpec([]),
            'result' => new EuropaplusResult([]),
        ]);
        $ctx->out['result'] = $preset;
        [$got, $err] = EuropaplusMakeResult::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_request_guards_a_missing_spec(): void
    {
        ...
        [, $err] = EuropaplusMakeRequest::call($ctx);
    ...
    {
        $utility = self::utility();
        $boom = new EuropaplusError('boom', 'boom');
        $utility->fetcher = function (EuropaplusContext $_c, string $_u, array $_f) use ($boom): array {
            return [null, $boom];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new EuropaplusSpec(['step' => 's', 'method' => 'GET', 'base' => 'http://h', 'parts' => ['a']]),
        ]);
        [$response, $err] = EuropaplusMakeRequest::call($ctx);
        $this->assertNull($err);
    ...
    {
        $utility = self::utility();
        $utility->fetcher = function (EuropaplusContext $_c, string $_u, array $_f): array {
            return [null, null];
        };
        $ctx = self::ctx([
            'utility' => $utility,
    ...
    public function test_make_request_a_normal_transport_response_is_wrapped(): void
    {
        ...
        [$response, $err] = EuropaplusMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertInstanceOf(EuropaplusResponse::class, $response);
    ...
    public function test_make_request_records_the_fetchdef_to_ctrl_explain(): void
    {
        ...
        $ctx->ctrl->explain = ['on' => true];
        EuropaplusMakeRequest::call($ctx);
    ...
    {
        $utility = self::utility();
        $utility->make_fetch_def = function (EuropaplusContext $c): array {
            return [null, $c->make_error('fetchdef_boom', 'boom')];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new EuropaplusSpec(['step' => 's', 'method' => 'GET']),
        ]);
        [$response, $err] = EuropaplusMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertNotNull($response->err);
    ...
    {
        $preset = new EuropaplusResponse(self::resp(201));
        $ctx = self::ctx(['spec' => new EuropaplusSpec([])]);
        $ctx->out['request'] = $preset;
        [$got, $err] = EuropaplusMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null]);
        [, $err] = EuropaplusMakeFetchDef::call($ctx);
    ...
    public function test_make_fetch_def_serialises_body_and_inits_missing_result(): void
    {
        ...
        [$fetchdef, $err] = EuropaplusMakeFetchDef::call($ctx);
        $this->assertNull($err);
    ...
    {
        $ctx = self::ctx(['result' => new EuropaplusResult(['ok' => true, 'resdata' => 42])]);
        $this->assertSame(42, EuropaplusDone::call($ctx));
    }

    public function test_done_raises_the_error_when_not_ok(): void
    {
        $ctx = self::ctx(['result' => new EuropaplusResult(['ok' => false])]);
        $this->expectException(EuropaplusError::class);
        EuropaplusDone::call($ctx);
    }

    public function test_make_error_returns_resdata_when_ctrl_throw_is_false(): void
    {
        $ctx = self::ctx(['result' => new EuropaplusResult(['ok' => false, 'resdata' => 'fallback'])]);
        $ctx->ctrl->throw_err = false;
        $this->assertSame('fallback', EuropaplusMakeError::call($ctx, null));
    }
    ...
    public function test_make_error_records_to_ctrl_explain(): void
    {
        ...
        $ctx->ctrl->throw_err = false;
        $ctx->ctrl->explain = ['on' => true];
        EuropaplusMakeError::call($ctx, null);
    ...
    public function test_feature_add_appends_in_call_order(): void
    {
        ...
        $a = new EuropaplusBaseFeature();
        $b = new EuropaplusBaseFeature();
        EuropaplusFeatureAdd::call($ctx, $a);
        EuropaplusFeatureAdd::call($ctx, $b);
        $this->assertSame([$a, $b], $client->features);
    }

    private static function named_feature(string $name): EuropaplusBaseFeature
    {
        $f = new EuropaplusBaseFeature();
        $f->name = $name;
        return $f;
    }
    ...
    public function test_feature_add_ordering_before_after_replace(): void
    {
        ...
        EuropaplusFeatureAdd::call($ctx, $miss);
    ...
    private static function auth_ctx(array $options, ?array $headers): EuropaplusContext
    {
        $client = new PlClient($options);
        return self::ctx([
            'client' => $client,
            'spec' => $headers === null ? null : new EuropaplusSpec(['headers' => $headers]),
        ]);
    }
    ...
    public function test_prepare_auth_guards_a_missing_spec(): void
    {
        ...
        [, $err] = EuropaplusPrepareAuth::call($ctx);
    ...
    public function test_prepare_auth_a_raw_apikey_goes_in_as_is(): void
    {
        ...
        EuropaplusPrepareAuth::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'response' => new EuropaplusResponse(['headers' => null]),
            'result' => new EuropaplusResult([]),
        ]);
        EuropaplusResultHeaders::call($ctx);
