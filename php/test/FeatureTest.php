...
class FtCtrl extends EuropaplusControl
{
...
class FtClient
{
    ...
    private EuropaplusUtility $utility;

    public function __construct(EuropaplusUtility $utility, array $options)
    {
    ...
    public function get_utility(): EuropaplusUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (EuropaplusContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public EuropaplusContext $rootctx;
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (EuropaplusContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = EuropaplusConfig::make_feature($name);
    ...
    private static function buildUrl(EuropaplusSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof EuropaplusError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new EuropaplusResponse($fetched);
        }
    ...
    private function fail(EuropaplusContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (EuropaplusContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
