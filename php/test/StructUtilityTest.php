...
// Europaplus SDK struct utility test
//
...
class StructUtilityTest extends TestCase
{
    ...
    {
        if (self::$run === null) {
            $runner = EuropaplusOmni::makeStructRunner(
        ...
        $this->runsection(
            ...
            function ($input) {
                ...
                if (is_object($store) && $haspath && Struct::undef() !== $val) {
                    ...
                    if (json_encode(EuropaplusOmni::structfix($now, Struct::undef()))
                        !== json_encode(EuropaplusOmni::structfix($val, Struct::undef()))) {
